...
classdef FeMeshSurface2D < AbsPostProcessingMeshFactory
    
    methods( Access = public )
        
        function obj = FeMeshSurface2D( gridSizes )
    ...
    methods( Access = private )
        ...
        function [vertices, tris] = getSurfaceMesh( obj, FeMesh  )
            ...
            sections = ceil( endPoint(1:2) - origin(1:2))./obj.GridSizes;
            
            x1D = linspace( origin(1), endPoint(1), sections(1) );
            y1D = linspace( origin(2), endPoint(2), sections(2) );          
            
            
            [vertices, tris] = obj.getXYMesh(FeMesh, x1D, y1D );
