...
function Lines = getBoundaryWithVisual(obj)
...
for j=1:length(subDomains)
    ...
    % correcting lines generated with SpaceTree
    if (not(isempty(c)))
        ...
        % plot the line segments representing the boundary for visual control
        hold on;
        line([c(1,(i+1));c(1,(i+2))],[c(2,(i+1)); c(2,(i+2))]) %plots lines
        end
    end
end
Lines=retLines;
